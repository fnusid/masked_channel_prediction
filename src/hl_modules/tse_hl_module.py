...
class PLModule(object):
    def __init__(self, sr, 
                 tsh_model, tsh_model_params,
                 enrollment_model, enrollment_model_params,
    ...
    def on_epoch_end(self, best_path, wandb_run):
        ...
        def log_audio(run, key, samples, sr):
            columns = ['mixture', 'enrollment', 'target', 'output']
            wandb_samples = []
    ...
    def _step(self, batch, batch_idx, step='train'):
        ...
        input_dict = {'mixture':inputs['mixture'], 'embedding':targets['embedding_gt']}
